...
def test_generate_labels_adds_manager_targets():
    ...
    assert "us100_trend_continuation_trigger_bar_index" in labeled.columns
    assert "us100_trend_continuation_earliest_tradable_bar_index" in labeled.columns
...
def test_label_leakage_check_passes_on_generated_labels():
    size = 260
    frame = pd.DataFrame(
        {
            "us100_high": [101.0] * size,
            "us100_low": [99.0] * size,
            "us100_close": [100.0] * size,
            "us100_atr_15": [1.0] * size,
            "us100_slope_15": [0.3] * size,
    ...
    setup_names = ["trend_continuation"]
    labeled = generate_labels(frame, LabelConfig(), setup_names)
    violations = run_label_leakage_check(labeled, setup_names)
    assert violations == []
